...
import { messageSchema } from "../src/validators/messageValidators";
import {
  getMessageById,
  getMessages,
  updateMessage,
...
import upload from "../src/middlewares/imagemid";
...
describe("comments APIs", () => {
  ...
  it("get a comments", async () => {
    const result = await supertest(app).get(`/api/blogs/${blogId}/comments`);
...
describe("createLike", () => {
  ...
  beforeEach(() => {
    ...
    res = {
      status: jest.fn().mockReturnValue({ json: jest.fn() }) as unknown as (
        code: number
      ) => Response<any, Record<string, any>>, // Explicitly defining the type
    };
  });
...
describe("blogValidationSchema", () => {
  ...
  it("should invalidate a blog object with short content", () => {
    ...
    expect(validationResult.error).toBeDefined(); // Error should be present for invalid input
    expect(validationResult.error?.message).toContain(
      '"content" length must be at least 30 characters long'
    );
  });
...
//     expect(res.status).toHaveBeenCalled(200); // Corrected status code to 200
...
describe("updateMessage", () => {
  it("should update a message when found", async () => {
    ...
    jest
      ...
      .mockResolvedValueOnce(updatedMessage as any);
...
describe("deleteMessage", () => {
  ...
  it("should handle errors and return 500 status code", async () => {
    ...
    jest
      .spyOn(Message, "findByIdAndDelete")
      .mockRejectedValueOnce(new Error("Database error"));
